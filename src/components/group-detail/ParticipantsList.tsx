...
export default function ParticipantsList({
  ...
  const hasAssignments = participants.some((participant) =>
    Boolean(participant.assignedParticipantName),
  );
    ...
    startRemoveTransition(async () => {
      try {
        ...
        setListError(
          error instanceof Error
            ? error.message
            : "Kon deelnemer niet verwijderen.",
        );
      } finally {
        setRemovingId(null);
      }
  ...
  const handleCopyShareLink = async (participant: GroupDetailParticipant) => {
    ...
    try {
      ...
      setListError(
        error instanceof Error ? error.message : "Kon link niet kopiëren.",
      );
    }
  };
  ...
  const handleExportToExcel = async () => {
    ...
    try {
      ...
      XLSX.writeFile(workbook, `lootjes-${timestamp}.xlsx`, {
        bookType: "xlsx",
      });
    } catch (error) {
      setListError(
        error instanceof Error ? error.message : "Kon lootjes niet exporteren.",
      );
    } finally {
  ...
  return (
    ...
    >
      <div className="flex flex-col gap-4">
          ...
          <div className="space-y-1">
            ...
            <div className="flex flex-col gap-2 sm:flex-row sm:items-start sm:justify-between">
              <p className="text-xs font-semibold text-black/60 uppercase">
                {participants.length}{" "}
                {participants.length === 1 ? "persoon" : "personen"} aangemeld
              </p>
              {hasAssignments ? (
          ...
          <AnimatePresence mode="wait" initial={false}>
            {participants.length === 0 ? (
              <motion.div
                ...
                className="border-4 border-dashed border-black px-6 py-10 text-center text-lg font-black text-black/60 uppercase"
              >
                    ...
                    >
                      ...
                      >
                        ...
                        {!canEdit ? (
                          ...
                          >
                            <p className="text-[10px] font-semibold tracking-[0.2em] text-black/60 uppercase">
                              Persoonlijke link
                            </p>
        ...
        {listError ? (
          <p className="text-sm font-semibold text-red-600">{listError}</p>
        ) : null}
      </div>
      ...
      >
        ...
        >
          <DialogHeader className="space-y-3">
            ...
            <DialogDescription className="text-sm font-semibold text-black/60 uppercase">
              {pendingVisibility
