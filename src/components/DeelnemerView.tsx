...
type RevealExperienceProps = Extract<
    ...
    <RevealShell>
      ...
      >
        <motion.div
          ...
          className="mx-auto w-full max-w-2xl -translate-2 rounded-xl border-8 border-black bg-white px-6 py-8 text-center shadow-[18px_18px_0px_rgba(0,0,0)] sm:px-10 sm:py-12"
        >
          <motion.div
          ...
          <div className="mt-6 space-y-3">
            {groupName ? (
              <motion.p
                ...
                className="inline-block rounded-md border-4 border-black bg-green-600 px-6 py-3 text-lg font-black tracking-wide text-white uppercase"
              >
                🎄 {groupName}
              </motion.p>
            ) : null}
            <motion.h1
              ...
              className="text-4xl font-black tracking-tight text-black uppercase sm:text-5xl"
            >
              Hoi, {participantName}!
            </motion.h1>
          </div>
            ...
            <AnimatePresence mode="wait">
              {!revealed ? (
                <motion.div
                  ...
                  className="space-y-8"
                >
                  <div className="mx-auto flex h-30 w-30 items-center justify-center rounded-full border-8 border-yellow-400 bg-black p-5 md:h-50 md:w-50 md:p-10">
                    <Lock className="h-full w-full text-yellow-300" />
                  </div>
                  <div className="space-y-4">
                    <h2 className="text-3xl leading-tight font-black text-balance text-black uppercase sm:text-4xl">
                      Jouw lootje is nog verborgen
                    </h2>
                    <p className="mx-auto max-w-md text-sm font-semibold tracking-widest text-balance text-neutral-700 uppercase">
                      Druk op de knop hieronder om te ontdekken wie jij hebt
                      getrokken!
                    </p>
                  </div>
                  <Button
                    onClick={reveal}
                    className="group inline-flex items-center gap-3 border-8 border-black bg-red-600 px-12 py-8 font-black tracking-wide !text-wrap text-white uppercase shadow-[12px_12px_0px_rgba(0,0,0,0.5)] transition-all hover:-translate-y-1 hover:shadow-[16px_16px_0px_rgba(0,0,0,0.35)] sm:py-6"
                    size="lg"
                  >
                ...
                >
                  ...
                  >
                    <span
                      className="absolute top-3 right-4 text-4xl opacity-40"
                      aria-hidden="true"
                    >
                      🎄
                    </span>
                    <span
                      className="absolute bottom-2 left-4 text-3xl opacity-40"
                      aria-hidden="true"
                    >
                      ⭐
                    </span>
                    <p className="text-sm font-black tracking-[0.4em] text-yellow-200 uppercase">
                      Jij hebt getrokken:
                    </p>
                    <h2 className="mt-4 text-4xl leading-tight font-black text-white uppercase sm:text-5xl">
                      {assignedParticipantName}
                    </h2>
                  </motion.div>
                  ...
                  <motion.p
                    ...
                    className="mx-auto max-w-lg rounded-lg border-4 border-black bg-white/90 p-2 text-sm font-black tracking-[0.3em] text-red-600 uppercase"
                  >
...
function StateCard({ icon, title, message }: StateCardProps) {
  return (
    ...
    >
      <div className="mx-auto mb-6 flex h-24 w-24 items-center justify-center rounded-full border-8 border-red-200 bg-red-600">
        {icon}
      </div>
      <h1 className="text-3xl font-black tracking-tight text-black uppercase sm:text-4xl">
        {title}
      </h1>
      <p className="mt-4 text-base font-semibold tracking-widest text-neutral-700 uppercase">
        {message}
      </p>
    </motion.div>
  );
...
function RevealShell({ children }: RevealShellProps) {
  return (
    <div className="relative min-h-screen overflow-hidden bg-yellow-400 text-white">
      ...
      />
      <div className="relative z-10 flex min-h-screen -rotate-1 items-center justify-center px-4 py-12 sm:px-8">
        {children}
      </div>
    </div>
  );
